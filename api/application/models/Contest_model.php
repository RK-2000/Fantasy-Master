...
class Contest_model extends CI_Model
{
    ...
    function addContest($Input = array(), $SessionUserID, $MatchID, $SeriesID, $StatusID = 1)
    {
        ...
        foreach ($MatchID as $Match) {
            ...
            $InsertData = array_filter(array(
                ...
                "NoOfWinners" => (@$Input['WinningAmount'] > 0) ? ((@$Input['ContestFormat'] == 'League') ? @$Input['NoOfWinners'] : 1) : 0,
    ...
    function getContests($Field = '', $Where = array(), $multiRecords = FALSE, $PageNo = 1, $PageSize = 15)
    {
        ...
        if (!empty($Field)) {
            ...
            $FieldArray = array(
                ...
                'CreatedType' => 'IF(C.PredraftContestID IS NULL, "Manually", "PreDraft") CreatedType',
    ...
    */
    ...
    function getJoinedContestsUsersMongoDB($Where = array(), $PageNo = 1, $PageSize = 15)
    {
        ...
        $JoinedContestsUsers = json_decode(json_encode(iterator_to_array($ContestCollection->find([], ['projection' => ['_id' => 0, 'UserGUID' => 1, 'UserTeamName' => 1, 'Username' => 1, 'FullName' => 1, 'ProfilePic' => 1, 'TotalPoints' => 1, 'UserTeamPlayers' => 1, 'UserRank' => 1, 'UserWinningAmount' => 1,'TaxAmount' => 1], 'skip' => paginationOffset($PageNo, $PageSize), 'limit' => (int) $PageSize, 'sort' => ['UserRank' => 1]]))),TRUE);
