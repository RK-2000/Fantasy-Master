...
class Sports_model extends CI_Model {
    ...
    function getPlayerPoints($CronID) {
        ...
        $LiveMatches = $this->getMatches('MatchID,MatchType,MatchScoreDetails,StatusID,IsPlayerPointsUpdated', array('Filter' => 'Yesterday', 'StatusID' => array(2, 5, 10), 'IsPlayerPointsUpdated' => 'No', 'OrderBy' => 'M.MatchStartDateTime', 'Sequence' => 'DESC'), true, 1, 10);
        /* Get Live Matches Data */
        if (!empty($LiveMatches)) {

            /* Delete Cache Key */
            $this->cache->memcached->delete('getJoinedContestPlayerPoints');
    ...
    function getJoinedContestTeamPoints_OLD1($CronID, $MatchID = "", $StatusArr = array(2)) {
    ...
    function getJoinedContestTeamPoints_OLD_MYSQL($CronID, $MatchID = "", $StatusArr = array(2)) {
        ...
        if ($Data->num_rows() > 0) {
            ...
            foreach ($Data->result_array() as $Value) {
                ...
                /* Update User Rank */
                if(!in_array($Value['ContestID'],$ContestIdArr)){
        ...
        if ($Data->num_rows() > 0) {
            ...
            foreach ($Data->result_array() as $Key => $Value) {
                ...
                $ContestCollection->updateOne(
                                ...
                                ['$set'   => ['ContestID' => $Value['ContestID'],'UserID' => $Value['UserID'],'UserTeamID' => $Value['UserTeamID'],'UserTeamName' => $Value['UserTeamName'],'Username' => $Value['Username'],'FullName' => $Value['FullName'],'ProfilePic' => $Value['ProfilePic'],'TotalPoints' => $UserTotalPoints,'UserTeamPlayers' => $UserPlayersArr,'IsWinningAssigned' => 'No']],
                                ['upsert' => true]
                            );
            }
...
//                        }
    ...
    function getPlayersLiveCricketApi($CronID) {
        ...
        /* Get matches data */
        $MatchesData = $this->getMatches('MatchStartDateTime,MatchIDLive,MatchID,MatchType,SeriesIDLive,SeriesID,TeamIDLiveLocal,TeamIDLiveVisitor,LastUpdateDiff', array('StatusID' => array(1, 2)), true, 1, 15);
        if (!$MatchesData) {
            $this->db->where('CronID', $CronID);
            $this->db->limit(1);
            $this->db->update('log_cron', array('CronStatus' => 'Exit', 'CronResponse' => $this->db->last_query()));
    ...
    function getPlayerStatsLiveCricketApi($CronID) {
        ...
        /* To get All Player Stats Data */
        $MatchData = $this->getMatches('MatchID,MatchIDLive,SeriesIDLive,SeriesID,MatchStartDateTime', array('StatusID' => 5, 'PlayerStatsUpdate' => 'No', 'MatchCompleteDateTime' => date('Y-m-d H:i:s')), true, 0);
        if (!$MatchData) {
            $this->db->where('CronID', $CronID);
            $this->db->limit(1);
            $this->db->update('log_cron', array('CronStatus' => 'Exit'));
    ...
    function getMatchScoreLiveCricketApi($CronID) {
        ...
        /* Get Live Matches Data */
        $LiveMatches = $this->getMatches('MatchIDLive,MatchID,MatchStartDateTime,Status,IsPlayingXINotificationSent,TeamNameShortLocal,TeamNameShortVisitor', array('Filter' => 'Yesterday', 'StatusID' => array(1, 2)), true, 1, 10);
        //$LiveMatches = $this->getMatches('MatchIDLive,MatchID,MatchStartDateTime,Status,IsPlayingXINotificationSent,TeamNameShortLocal,TeamNameShortVisitor', array('MatchID' => 1313), true, 1, 10);
        //print_r($LiveMatches);exit;
        if (!$LiveMatches) {
            $this->db->where('CronID', $CronID);
            $this->db->limit(1);
            $this->db->update('log_cron', array('CronStatus' => 'Exit', 'CronResponse' => $this->db->last_query()));
