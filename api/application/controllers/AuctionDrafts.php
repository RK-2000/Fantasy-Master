...
class AuctionDrafts extends API_Controller {
    ...
    public function getContests_post() {
        ...
        /* Get Contests Data */
        $ContestData = $this->AuctionDrafts_model->getContests(@$this->Post['Params'], array_merge($this->Post, array('LeagueType' => 'Auction', 'SeriesID' => $this->SeriesID, 'UserID' => @$this->UserID, 'SessionUserID' => $this->SessionUserID, 'AuctionStatusID' => @$this->StatusID)), TRUE, @$this->Post['PageNo'], @$this->Post['PageSize']);
    ...
    public function getPlayersAuction_post() {
        ...
        $this->form_validation->validation($this);  /* Run validation */
        /* Get Players Data */
        $PlayersData = $this->AuctionDrafts_model->getPlayersAuction(@$this->Post['Params'], array_merge($this->Post, array('SeriesID' => $this->SeriesID, 'ContestID' => @$this->ContestID, 'SessionUserID' => @$this->SessionUserID, 'PlayerID' => @$this->PlayerID)), TRUE, @$this->Post['PageNo'], @$this->Post['PageSize']);
        if (!empty($PlayersData)) {
            $this->Return['Data'] = $PlayersData['Data'];
        }
    }
    ...
    public function addAuctionPlayerBid_post() {
        ...
        $AuctionBid = $this->AuctionDrafts_model->addAuctionPlayerBid($this->Post, $UserID, $this->SeriesID, $this->ContestID, $this->PlayerID);
    ...
    public function getPlayerBid_post() {
        ...
        if (!isset($AuctionGames['Data']['Records'])) {
            $playersData = $this->AuctionDrafts_model->getPlayers($this->Post['Params'], array_merge($this->Post, array('SeriesID' => $this->SeriesID, 'ContestID' => $this->ContestID, 'PlayerBidStatus' => 'Yes', 'PlayerStatus' => 'Live', 'OrderBy' => "PlayerID", "Sequence" => "ASC")));
            if (empty($playersData)) {
                $playersData = $this->AuctionDrafts_model->getPlayers($this->Post['Params'], array_merge($this->Post, array('SeriesID' => $this->SeriesID, 'ContestID' => $this->ContestID, 'PlayerBidStatus' => 'Yes', 'PlayerStatus' => 'Upcoming', 'OrderBy' => "PlayerID", "Sequence" => "ASC")));
            }
        } else {
            $playersData = $this->AuctionDrafts_model->getPlayers($this->Post['Params'], array_merge($this->Post, array('SeriesID' => $this->SeriesID, 'ContestID' => $this->ContestID, 'OrderBy' => "PlayerID", "Sequence" => "ASC")));
        }
        if (!empty($playersData)) {
    ...
    public function getPrivateContest_post() {
        ...
        $ContestData = $this->AuctionDrafts_model->getContests('ContestID,LeagueJoinDateTime', array_merge($this->Post, array('UserInvitationCode' => @$this->Post['UserInvitationCode'])), FALSE, @$this->Post['PageNo'], @$this->Post['PageSize']);

        if (!empty($ContestData['ContestGUID'])) {
            $validateContest = $this->joinPrivateLeague($ContestData['ContestGUID'],$ContestData['LeagueJoinDateTime']);
            $this->Return['ResponseCode'] = $validateContest['ResponseCode'];
            $this->Return['Message'] = $validateContest['Message'];
        }else{
            $this->Return['ResponseCode'] = 500;
            $this->Return['Message'] = "Invaild League Code.";
        }
    }

    public function joinPrivateLeague($ContestGUID,$LeagueJoinTime)
    {
        $_POST['ContestGUID'] = $ContestGUID;
        $this->form_validation->set_rules( 'ContestGUID' , 'ContestGUID', 'trim|required|callback_validateEntityGUID[Contest,ContestID]|callback_validateUserJoinContest');
        $this->form_validation->validation($this);  /* Run validation */

        $Return = array();
        $LeagueJoinDateTime = $LeagueJoinTime;
        $currentDateTime = date('Y-m-d H:i:s');

        if ($LeagueJoinDateTime < $currentDateTime) {
            $this->AuctionDrafts_model->changeContestStatus($this->ContestID);
            $Return['ResponseCode'] = 500;
            $Return['Message'] = "You can not join the contest because time is over.";
        } else {
            $JoinContest = $this->AuctionDrafts_model->joinContest($this->Post, $this->SessionUserID, $this->ContestID);
            if (!$JoinContest) {
                $Return['ResponseCode'] = 500;
    ...
    public function getJoinedContestsUsers_post() {
        ...
        $JoinedContestData = $this->AuctionDrafts_model->getJoinedContestsUsers(@$this->Post['Params'], array('ContestID' => @$this->ContestID, 'SeriesID' => @$this->SeriesID, 'SessionUserID' => @$this->SessionUserID), TRUE, @$this->Post['PageNo'], @$this->Post['PageSize']);
    ...
    public function getContestBidHistory_post() {
        ...
        /* Get Joined Contest Users Data */
        $JoinedContestData = $this->AuctionDrafts_model->getContestBidHistory(@$this->Post['Params'], array('ContestID' => @$this->ContestID, 'SeriesID' => @$this->SeriesID, 'PlayerID' => @$this->PlayerID), TRUE, @$this->Post['PageNo'], @$this->Post['PageSize']);
        if (!empty($JoinedContestData)) {
            $this->Return['Data'] = $JoinedContestData['Data'];
        }
    }
    ...
    public function auctionTeamPlayersSubmit_post() {
        ...
        $this->form_validation->set_rules('UserTeamPlayers', 'UserTeamPlayers', 'trim');
        if (!empty($this->Post['UserTeamPlayers']) && is_array($this->Post['UserTeamPlayers'])) {
            if (count($this->Post['UserTeamPlayers']) > 16) {
                $this->Return['ResponseCode'] = 500;
                $this->Return['Message'] = "Team Players length can't greater than 16.";
                exit;
            }
        ...
        if (!$this->AuctionDrafts_model->auctionTeamPlayersSubmit($this->Post, $this->UserTeamID, $this->SeriesID)) {
            $this->Return['ResponseCode'] = 500;
            $this->Return['Message'] = "An error occurred, please try again later.";
        } else {
            $this->Return['Message'] = "Team updated successfully.";
