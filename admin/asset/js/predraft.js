app.controller('PageController', function ($scope, $http, $timeout, $rootScope) {
    ...
    $scope.loadFormEdit = function (Position, PreContestID)
    {
        ...
        $http.post(API_URL + 'admin/preContest/getPreContest', 'SessionKey=' + SessionKey + '&PreContestID=' + PreContestID + '&Params=unfilledWinningPercent,IsVirtualUserJoined,VirtualUserJoinedPercentage,AdminPercent,Privacy,IsPaid,WinningAmount,ContestSize,EntryFee,NoOfWinners,EntryType,CustomizeWinning,ContestType,CashBonusContribution,UserJoinLimit,ContestFormat,IsConfirm,ShowJoinedContest,IsAutoCreate',contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
                $scope.data.pageLoading = false;
                $scope.formData = response.Data.Records[0]
            console.log(response.Data.Records[0].CustomizeWinning.length);


                $scope.WinningAmount = parseInt(response.Data.Records[0].WinningAmount);
    ...
    $scope.addData = function ()
    {
        ...
        if ($scope.contestPrizeParser($scope.custom.choices)[0].WinningAmount == 0) {
            var customWinings = JSON.stringify([{'From': 1, 'To': $scope.custom.NoOfWinners, 'WinningAmount': $scope.WinningAmount, 'percent': 100}]);
        } else {
    ...
    $scope.addDataPreContest = function ()
    {
        ...
        $http.post(API_URL + 'admin/preContest/add', $.param($data), contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
                alertify.success(response.Message);
                $scope.applyFilter();
    ...
    $scope.editData = function ()
    {
        ...
        inputData.WinningAmount = $scope.WinningAmount;
    ...
    $scope.$watch('custom.EntryFee', function (newValue, oldValue) {
        if (newValue != oldValue) {
            ...
            if (parseInt($scope.custom.ContestSize) > 0) {
                ...
                $scope.totalEntry = $scope.custom.EntryFee * $scope.custom.ContestSize;
      ...
      $scope.$watch('custom.ContestSize', function (newValue, oldValue) {
        ...
        if (newValue != oldValue) {
            ...
            if (typeof $scope.custom.EntryFee == 'undefined') {
                $scope.EntryFee_error = true;
                return false;
            } else {
                $scope.EntryFee_error = false;
            }
            /*if (newValue > 100) {
    ...
    $scope.changePercent = function (x) {
        ...
        if (x != 0 && x > 0) {
            ...
            if ((parseFloat(($scope.WinningAmount * $scope.custom.choices[x].percent) / 100) / tempPersnCount1) > (parseFloat($scope.WinningAmount * $scope.custom.choices[x - 1].percent / 100) / tempPersnCount0)) {
        ...
        for (var i = 0; i < $scope.custom.choices.length; i++) {
            if (i === x) {
                ...
                $scope.winnersAmount = $scope.WinningAmount * $scope.custom.choices[i].percent / 100;
    ...
    $scope.removeField = function (index) {
        ...
        for (var i = 0; i < $scope.custom.choices.length; i++) {
              //total = total;
            console.log($scope.custom.choices[i].percent + "percent")
            console.log(typeof parseFloat($scope.custom.choices[i].percent) + "type")
            if($scope.custom.choices[i].percent === "")
            {
                console.log("dffd");
                $scope.custom.choices[i].percent = parseFloat(0);
